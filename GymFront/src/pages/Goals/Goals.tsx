  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", paddingBottom: 64 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 48, marginTop: 16, borderBottom: "1px solid var(--border)", paddingBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 40, fontWeight: 800, color: "var(--ink)", letterSpacing: "-0.04em", textTransform: "uppercase", lineHeight: 1 }}>{t("targetObjectives").toUpperCase()}</h1>
        </div>
        <button 
      ...
      {loading && <div style={{ color: "var(--muted)", fontFamily: "var(--mono)" }}>[ {t("goals").toUpperCase()}... ]</div>}

      {goals && goals.length === 0 && <div className="card" style={{ fontFamily: "var(--mono)", color: "var(--muted)" }}>[ {t("noGoals").toUpperCase()} ]</div>}

      {goals && goals.length > 0 && (
        <>
          <div style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--muted)", fontWeight: 700, marginBottom: 16 }}>{t("activeTargets").toUpperCase()}</div>
          <div className="grid g-3">
            {goals.map((goal) => {
              ...
              return (
                ...
                >
                  <div style={{ fontWeight: 800, fontSize: 18, marginBottom: 4, color: "var(--ink)" }}>{goal.exerciseName}</div>
                  <div style={{ fontSize: 13, color: "var(--muted)", fontFamily: "var(--mono)", marginBottom: 16, letterSpacing: "0.05em" }}>
                    {t("target").toUpperCase()}: <span style={{ color: "var(--ink)" }}>{goal.targetWeightKg}KG x {goal.targetReps}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontFamily: "var(--mono)", fontWeight: 700, color: pct >= 100 ? "var(--success)" : "var(--accent)" }}>
                    ...
                    <span style={{ fontSize: 11, letterSpacing: "0.1em" }}>{pct >= 100 ? t("completed") : t("inProgress")}</span>
                  </div>
                </div>
              );
            })}
